--pig -x local -f test.pig -param filename=/Users/vera.kalinichenko/Downloads/t/dataset.txt
...
Items = filter A by (item == 93920 OR item == 68411 OR item==35355 OR item==3299 OR item== 58302);
ItemsD = distinct Items;

agrpd = GROUP ItemsD BY item;

itemsJoined = JOIN ItemsD BY item, agrpd BY group;
...
--store itemsJoinedGrp into 'f612/items_joinedGrp';
--store itemsOneId into 'f612/itemsOneId';
...
common3 = foreach common2 generate TOTUPLE($0), $1 as cos_sim;
...
d = foreach data generate item1, item2, overlap(idsL, idsR) as top, (double)SQRT(countL*countR) as bottom;
result = foreach d generate TOTUPLE(item1, item2), (double)(top/bottom);
store result into 'feb612/result3';